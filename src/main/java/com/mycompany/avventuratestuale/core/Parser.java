...
/**
 * Parser verbo-oggetto con stopword e suggerimenti fuzzy basati su Levenshtein.
 */
public class Parser {
    ...
    /**
     * Interpreta l'input dell'utente individuando comando, oggetto primario e oggetto secondario.
     *
     * @param input testo scritto dal giocatore
     * @param comandi dizionario dei comandi disponibili
     * @param oggettiStanza oggetti visibili nella stanza corrente
     * @param inventario oggetti posseduti dal giocatore
     * @return risultato del parsing, oppure null se l'input è vuoto
     */
    ...
    private Oggetto cercaOggetto(String token, List<Oggetto> oggettiStanza, List<Oggetto> inventario) {
        Oggetto trovato = oggettiStanza.stream()
                .filter(o -> o.isVisibile() && (o.getNome().equalsIgnoreCase(token) || o.getSinonimi().contains(token)))
                .findFirst()
                .orElse(null);
    ...
    /**
     * Suggerisce il comando più simile a una parola non riconosciuta.
     *
     * @param parola token digitato dal giocatore
     * @param comandi comandi e sinonimi disponibili
     * @return sinonimo più vicino se la distanza è bassa, altrimenti null
     */
